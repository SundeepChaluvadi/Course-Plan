...
def populate_sortlist(tmplist,poplist,core_courses,core_prereq):
    for i in poplist:
        #print("pop",i.class_deptnumber,i.dept_core)
        if i.department=='Gen.Ed':
            i.dept_core='Gen.Ed'
        elif i.dept_core=="Free_Elective":
            i.dept_core="Free_Elective"
        elif i.class_deptnumber in core_courses:
            i.dept_core="Core_Course"
        elif i.class_deptnumber in core_prereq:
            i.dept_core="Core_Prereq"
        elif i.department=="CMP SCI" and int(i.class_number)>=3000:
...
def pass_available():
    ...
    courselist=populate_sortlist(courselist,prime_course_list,core_courses,core_prereq)
...
def run_project(semes,season,summ,creds,summer_creds,waived_courses,completed_courses):
    prime_course_list=xml_parser.populate_course_list('course_data_updated.xml')
    ...
    #for i in sortlist:
        #print(i.class_deptnumber,i.dept_core)
    ...
    if waived_courses==['']:
        ...
        for course in waived_courses:
            prewaived.append(deptnumber_to_course(sortlist,course))
    if completed_courses==['']:
        precomped=[]
    else:
        for course in completed_courses:
            precomped.append(deptnumber_to_course(sortlist,course))

    #Semesters
    sems=int(semes)
    #print("Printing semes",semes)
    sems=int(semes)
    #Starting season
    seas=season
    ...
    credits_persem=int(creds)
    #Credits per summer, default assumes half of credits_persem
    credits_persummer=int(summer_creds)
    #Starting Gened credits
    gened_credits=0
    starting_free=0
    ...
    calendar=classes.Calendar(seas,summer,sems,waived,comped,available,credits_persem,credits_persummer,gened_credits,starting_free)
    #print("len available: ",len(available.course_list))
