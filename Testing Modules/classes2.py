...
class Calendar:
    ...
    def move_course(self, course, from_semester_index, to_semester_index):
        ...
        if not any(course in val.class_deptnumber for val in self.semesters[from_semester_index]):
        ...
        if not (0 <= from_semester_index < len(self.semesters) and 0 <= to_semester_index < len(self.semesters)):
            ...
            return False
        #print(course)
        #print(self.semesters[from_semester_index].sem_course_names)
        # Check if the course exists in the 'from' semester
        if course not in self.semesters[from_semester_index].sem_course_names:
        ...
        if (to_semester.hours + course.credits) > self.credits_persem:
        ...
        self.semesters[from_semester_index].sem_courses.remove(course)
        self.semesters[from_semester_index].hours -= course.credits
        print("Removed from : ", self.semesters[:])

        # Add the course to the 'to' semester
        self.semesters[to_semester_index].sem_courses.append(course)
        self.semesters[to_semester_index].hours += course.credits
    ...
    def add_course(self, course_deptnumber, semester_index):
        ...
        self.available.pop_course(course_deptnumber)
    ...
    def remove_course(self, course, semester_index):
        ...
        if course not in semester.sem_courses:
            print("Course not found in the specified semester. Remove")
            return False
        semester.pop_course(course)
        # Add the course back to available courses
        self.available.add_course(course)
        return True