...
def semester_generator(sems,waived,completed):
    ...
    for i in range(sems):
        tmplist.append(classes_bhavya.Semester(i))
    return tmplist
...
file_path = r'course_data_updated.xml'
print("Current working directory:", os.getcwd())  # Print current working directory
print("File exists:", os.path.exists(file_path))  # Check if file exists
prime_course_list = xml_parser.populate_course_list(file_path)
...
available=classes_bhavya.Available(sortlist)
...
available=classes_bhavya.Available(sortlist)
waived=classes_bhavya.Waived(prewaived)
available.pop_courses(prewaived)
comped=classes_bhavya.Completed(precomped)
available.pop_courses(precomped)
print()

calendar=classes_bhavya.Calendar(seas,summer,sems,waived,comped,available,credits_persem)
calendar.load_courses(available)

calendar.print_values()

#Note: Courses added should be the position of the semester+1. This i
#This is to account for both the subtraction from the index combined with the
#addition of of the waived/completed course objects as pseudo semesters



calendar.move_course("CMP SCI 4500", 5, 6)
# Remove a course from a semester

calendar.print_values()

calendar.remove_course("MATH 1320", 3) 
# Add a course to a semester
#calendar.add_course("CMP SCI 5300", 4)
#Why was this CMP SCI 5300? Change to undergrad course
calendar.print_values()
calendar.add_course("CMP SCI 4300", 4)
calendar.print_values()
